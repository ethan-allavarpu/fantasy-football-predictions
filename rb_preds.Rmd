...
start_rb_test <- mutate(start_rb_test,
                        play_style = factor(assigned_cluster(start_rb_test,
                                                             best_rb_clust),
                                            levels = c("1", "2", "3", "4", "5", "6"))) %>% select(names(rb_vars))

final_x <- model.matrix(new_ff_scores ~ ., data = start_rb_test)[, -1]
final_preds <- predict(salary_lasso, final_x, s = lambda_1se)
