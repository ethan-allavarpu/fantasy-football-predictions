...
```{r}
set.seed(1)
qb_stats <- select(qbs, age:rush_td, fmb:two_pt_pass,
                   comp_pct:rush_ypg)
...
rb_stats <- scale(select(rbs, age:games_started, rush_att:two_pt_made,
                         rush_ypg:rec_ypg))
...
tes <- mutate(tes, rush_ypg = rush_yd / games,
              rec_ypg = rec_yd / games)
te_stats <- scale(select(tes, age:games_started, rush_att:two_pt_made,
                         rush_ypg:rec_ypg))
...
qb_preds <- predict(qb_bag, select(qb_validation, names(qb_vars)))
val_mse <- mean((qb_validation$new_ff_scores - qb_preds)^2)
cor(qb_validation$new_ff_scores, qb_preds)
val_mse / total_mse
qb_mses[3] <- val_mse
...
qb_val_rel_vars <- select(qb_validation, age:rush_td, fmb:two_pt_pass, comp_pct:rush_ypg, ppr, new_ff_scores, play_style)
...
qb_predictors <- qb_vars %>% select(age:ppr)
...
qb_bag <- randomForest(new_ff_scores ~ ., data = qb_new_vars,
                           mtry = p, importance = TRUE, ntree = 50)
qb_preds <- predict(qb_bag, select(qb_new_validation, names(qb_vars)))
val_mse <- mean((qb_validation$new_ff_scores - qb_preds)^2)
val_mse / total_mse
