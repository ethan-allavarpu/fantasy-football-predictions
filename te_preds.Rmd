...
boxplot(tes$ppr)
tes <- filter(tes, new_ff_scores <= 303.5, ppr <= 266.4)
tes <- mutate(tes, rush_ypg = rush_yd / games,
              rec_ypg = rec_yd / games)
te_stats <- select(tes, age:games_started, rush_att:two_pt_made, rush_ypg:rec_ypg)
...
te_stats <- select(tes, age:games_started, rush_att:two_pt_made,
                         rush_ypg:rec_ypg)
te_clusters <- best_k_means(te_stats, 2, 25)
...
te_preds <- predict(te_bag, select(te_validation, names(te_vars)))
val_mse <- mean((te_validation$new_ff_scores - te_preds)^2)
cor(te_validation$new_ff_scores, te_preds)
val_mse / total_mse
te_mses[3] <- val_mse
...
te_val_rel_vars <- select(te_validation, names(te_vars))
...
te_preds <- predict(te_bag, select(te_new_validation, names(te_new_vars)))
...
te_val_rel_vars <- te_val_rel_vars <- select(te_new_validation,
                                             names(te_new_vars))
test_x <- model.matrix(new_ff_scores ~ ., data = te_val_rel_vars)[, -1]
lambda_grid <- 10^(seq(from = 10, to = -2, length.out = 100))
salary_lasso <- glmnet(train_x, train_y, family = "gaussian",
...
```{r}
hist(exp(te_val_rel_vars$new_ff_scores), freq = FALSE, col = "blue", density = 20, ylim = c(0, 0.01))
hist(exp(val_preds), freq = FALSE, col = "red", density = 30, xlim = range(exp(val_preds), exp(te_val_rel_vars$new_ff_scores)), add = TRUE)

plot(exp(te_val_rel_vars$new_ff_scores), exp(te_val_rel_vars$new_ff_scores) - exp(val_preds))
train_preds <- exp(predict(salary_lasso, train_x, s = lambda_1se))
train_results <- exp(te_new_vars$new_ff_scores)
next_model <- lm(train_results ~ train_preds)

next_preds <- next_model$coefficients[1] + next_model$coefficients[2] * exp(val_preds)
test_results <- exp(te_val_rel_vars$new_ff_scores)
plot(next_preds, test_results - next_preds)
...
invResPlot(te_lm,
          ...
          las = 1)
summary(powerTransform(te_lm))

te_new_vars <- te_vars
te_new_vars$new_ff_scores <- (te_new_vars$new_ff_scores)^1.88
te_lm <- lm(new_ff_scores ~ ., data = te_new_vars)

te_preds <- (predict(te_lm, te_validation))^1/2
val_mse <- mean((te_validation$new_ff_scores - te_preds)^2)
val_mse / total_mse
te_mses[10] <- val_mse
names(te_mses)[10] <- "y_sqrt"
plot(te_preds, te_preds - te_validation$new_ff_scores)


te_predictors <- te_vars %>% select(age:rec_ypg)
summary(te_predictors)
te_predictors <- mutate(te_predictors,
                        games_started = games_started + 1,
                        rush_att = rush_att + 1,
                        rush_yd = rush_yd + 20,
                        rush_yd_per_att = rush_yd_per_att + 20,
                        rush_td = rush_td + 1,
                        fmb = fmb + 1,
                        fmb_lst = fmb_lst + 1,
                        rush_ypg = rush_ypg + 5) %>%
...
te_preds_trans <- summary(powerTransform(as.matrix(te_predictors) ~ 1))
te_preds_trans <- te_preds_trans$result[, 2]
te_new_vars <- te_vars
te_new_vars <- mutate(te_new_vars,
                        games_started = games_started + 1,
                        rush_td = rush_td + 1,
                        rec_td = rec_td + 1,
                        fmb = fmb + 1,
...
for (i in 1:18) {
  ith_var <- te_new_vars[[i]]
  trans <- te_preds_trans[i]
  if (trans == 0) {
    change <- log(ith_var)
  } else {
    change <- ith_var^trans
  }
  te_new_vars[[i]] <- change
}
te_lm <- lm(new_ff_scores ~ ., data = te_new_vars)

te_new_validation <- te_validation
te_new_validation <- mutate(te_new_validation,
                            games_started = games_started + 1,
                            rush_td = rush_td + 1,
                        rec_td = rec_td + 1,
                        fmb = fmb + 1,
...
te_bag <- randomForest(new_ff_scores ~ ., data = te_new_vars,
                           mtry = p, importance = TRUE, ntree = 50)
te_preds <- predict(te_bag, select(te_new_validation, names(te_vars)))
val_mse <- mean((te_validation$new_ff_scores - te_preds)^2)
val_mse / total_mse
...
train_x <- model.matrix(new_ff_scores ~ ., data = te_vars)[, -1]
train_y <- te_vars$new_ff_scores
te_val_rel_vars <- select(te_new_validation, names(te_vars))
test_x <- model.matrix(new_ff_scores ~ ., data = te_val_rel_vars)[, -1]
lambda_grid <- 10^(seq(from = 10, to = -2, length.out = 100))
salary_lasso <- glmnet(train_x, train_y, family = "gaussian",
                       alpha = 1, lambda = lambda_grid, standardize = FALSE)
...
plot(te_val_rel_vars$new_ff_scores, te_val_rel_vars$new_ff_scores - val_preds)
```
